...
export class PostTagRepository implements IPostTagRepository {
	...
	async findBySlug(slug: string): Promise<IPostTag | null> {
		const storedId = await this.cache.get(`${PostTag[EntitySource]}::${slug}`);
	...
	async update(postTag: IPostTag): Promise<void> {
		...
		if (_cachedPostTag) {
			...
			await this.cache.del(`${PostTag[EntitySource]}::$${cachedPostTag.id}`);
			await this.cache.del(`${PostTag[EntitySource]}::${cachedPostTag.slug}`);
		}
